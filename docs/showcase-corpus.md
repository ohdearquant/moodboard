# Reviewed public-domain visual corpus

The visual-retrieval workflow uses a reviewed 15-image corpus, not PACS, WikiArt, search-result
...
This is a small integration and retrieval corpus. It is not proof of broad aesthetic
...
id where applicable, title, artist, licence id/URL/public-domain confirmation, retrieval time,
optional ETag and Last-Modified value, byte count and image dimensions, SHA-256, and Khive
BlobStore v1 BLAKE3-256 `content_ref`. ETag is recorded when the server supplies it; a missing
...
tracked field or image byte still changes the manifest or blocks acquisition.
...
Met Open Access/CC0 boundary. The catalogue records the exact API response data expected for every
