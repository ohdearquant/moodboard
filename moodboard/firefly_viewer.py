...
BRIDGE_FORMAT = "moodboard.viewer-firefly-measured-loop-bridge.v1"
GENERATOR_REVISION = "moodboard.firefly-viewer-bridge.v1"
_BRIDGE_DOMAIN = b"moodboard-viewer-firefly-bridge-v1\0"
...
_PROJECTION_REVISION = "moodboard.adobe-demo-firefly-frozen-projection.v1"
_PREVIEW_SHA256 = {
...
def validate_viewer_firefly_bridge(value: Mapping[str, Any]) -> None:
    ...
    _closed(
        evidence,
        {"capture", "khive", "nonclaims", "projection", "replacement", "restyle"},
    ...
    expected_projection_sha256 = hashlib.sha256(
        (Path(__file__).parents[1] / "eval" / "adobe_demo_firefly_projection.py").read_bytes()
    ).hexdigest()
    ...
    if khive["namespace"] != "adobe-demo-firefly-v1":
    ...
    _closed(
        replacement,
        {"intent", "retrieved_reference", "threshold", "timeline", "verifier_revision"},
        "replacement",
    )
