...
export interface FireflySourceIdentity {
  readonly asset_id: "fruit_apple_garden";
  readonly byte_size: 645201;
...
export interface FireflyExactOutsideMask {
  readonly changed_pixel_count: 0;
  readonly comparison: "decoded_rgb_u8_outside_mask";
  readonly mask: {
    readonly bounds_half_open_source_pixels: {
      readonly x0_inclusive: 230;
      readonly x1_exclusive: 1152;
      readonly y0_inclusive: 48;
      readonly y1_exclusive: 912;
    };
    readonly encoding: "u8_row_major_1_inside_0_outside";
    readonly height: 960;
    readonly inside_pixel_count: 796608;
    readonly outside_pixel_count: 432192;
    readonly sha256: string;
    readonly width: 1280;
  };
  readonly max_abs_channel_error: 0;
  readonly result: "pass";
  readonly selected_output_sha256: string;
  readonly semantics: "deterministic_compositor_invariant_not_generator_locality";
...
export interface FireflyMeasuredEvidence {
  ...
  readonly khive: {
    ...
    readonly namespace: "showcase-firefly-v1";
    readonly restart: {
  ...
  readonly source: FireflySourceIdentity;
  readonly replacement: {
    readonly compositor_exact_outside_mask: FireflyExactOutsideMask;
...
interface FireflyBridge {
  ...
  readonly format_version: "moodboard.viewer-firefly-measured-loop-bridge.v2";
  readonly generator_revision: "moodboard.firefly-viewer-bridge.v2";
...
const sourceContentRef = "d9c1a0e3e6a5a72a9da252a0ea9fb4616c9099dd20cdc65ea00ffc29d14f23a8";
const sourceSha = "3bda38b4304152f813f6bea37dc236f95670fbea5da4731903d9ce8cfaa8ae23";
const maskSha = "09f9072f646ef8d99af30736210a57f2de448e8ca90fbff07a07edd7bd5eef4b";
...
function sourceIdentityAt(value: unknown): FireflySourceIdentity {
  const path = "$firefly.evidence.source";
  const source = objectAt(value, path);
  closed(
    source,
    ["asset_id", "byte_size", "content_ref", "height", "mime", "sha256", "width"],
    path,
  );
  if (
    source.asset_id !== "fruit_apple_garden"
    || source.byte_size !== 645201
    || shaAt(source.content_ref, `${path}.content_ref`) !== sourceContentRef
    || source.height !== 960
    || source.mime !== "image/jpeg"
    || shaAt(source.sha256, `${path}.sha256`) !== sourceSha
    || source.width !== 1280
  ) {
    throw new Error("$firefly source identity drifted");
  }
  return {
...
function exactOutsideMaskAt(value: unknown): FireflyExactOutsideMask {
  const path = "$firefly.evidence.replacement.compositor_exact_outside_mask";
  ...
  const mask = objectAt(exact.mask, `${path}.mask`);
  closed(
    mask,
    [
  ...
  const bounds = objectAt(
    mask.bounds_half_open_source_pixels,
    `${path}.mask.bounds_half_open_source_pixels`,
  );
  closed(
    bounds,
    ["x0_inclusive", "x1_exclusive", "y0_inclusive", "y1_exclusive"],
    `${path}.mask.bounds_half_open_source_pixels`,
  );
  if (
    exact.changed_pixel_count !== 0
    || exact.comparison !== "decoded_rgb_u8_outside_mask"
    || exact.max_abs_channel_error !== 0
    || exact.result !== "pass"
    || shaAt(exact.selected_output_sha256, `${path}.selected_output_sha256`) !== selectedOutputSha
    || exact.semantics !== "deterministic_compositor_invariant_not_generator_locality"
    || shaAt(exact.source_sha256, `${path}.source_sha256`) !== sourceSha
    || bounds.x0_inclusive !== 230
    || bounds.x1_exclusive !== 1152
    || bounds.y0_inclusive !== 48
    || bounds.y1_exclusive !== 912
    || mask.encoding !== "u8_row_major_1_inside_0_outside"
    || mask.height !== 960
    || mask.inside_pixel_count !== 796608
    || mask.outside_pixel_count !== 432192
    || shaAt(mask.sha256, `${path}.mask.sha256`) !== maskSha
    || mask.width !== 1280
  ) {
    throw new Error("$firefly exact outside-mask evidence drifted");
  }
  return {
    ...
    result: "pass",
    selected_output_sha256: selectedOutputSha,
...
function decodePythonPrevalidatedFireflyBridge(value: unknown): FireflyBridge {
  ...
  if (bridge.format_version !== "moodboard.viewer-firefly-measured-loop-bridge.v2" || bridge.generator_revision !== "moodboard.firefly-viewer-bridge.v2" || bridge.state !== "projected") {
    throw new Error("$firefly contract identity drifted");
  }
  const evidence = objectAt(bridge.evidence, "$firefly.evidence");
  closed(evidence, ["capture", "khive", "nonclaims", "projection", "replacement", "restyle", "source"], "$firefly.evidence");
  ...
  const source = sourceIdentityAt(evidence.source);
  const replacement = objectAt(evidence.replacement, "$firefly.evidence.replacement");
  closed(replacement, ["compositor_exact_outside_mask", "intent", "retrieved_reference", "threshold", "timeline", "verifier_revision"], "$firefly.evidence.replacement");
  if (replacement.threshold !== 0.95) throw new Error("$firefly threshold drifted");
  const compositorExactOutsideMask = exactOutsideMaskAt(
  ...
  return {
    ...
    evidence: {
      ...
      source,
      replacement: {
        compositor_exact_outside_mask: compositorExactOutsideMask,
    ...
    format_version: "moodboard.viewer-firefly-measured-loop-bridge.v2",
...
export const __test = { decodePythonPrevalidatedFireflyBridge };
