...
/**
 * Resolve only decoder-verified media. A bridge-owned source is preferred because Pixel RAG and
 * the board report may legitimately come from different governed datasets. Report ids remain a
 * compatibility fallback, but SHA-256 is always the byte identity: a stale id must never display
 * unrelated pixels.
 */
export function resolvePixelRagMediaSource(
  ...
  const embedded = verifiedPixelRagMediaSources.get(identity.content_sha256);
...
export const pixelRagArtifact = {
  format_version: 1,
  artifact_id: "pixel-rag:showcase:contract-v1",
...
function hitAt(value: unknown, path: string): PixelRagEvidenceHit {
  const record = objectAt(value, path);
  closed(record, ["rank", "source_search_rank", "asset_id", "content_sha256", "media", "khive", "title", "creator", "license", "score", "rationale"], path);
...
type PixelRagBridgeEnvelope =
  | {
      ...
      readonly format_version: "moodboard.viewer-pixel-rag-bridge.v2";
      readonly generator_revision: "moodboard.pixel-rag-viewer-bridge.v2";
      readonly input: null;
      readonly media: null;
  ...
  | {
      ...
      readonly media: PixelRagBridgeMedia;
      readonly state: "projected";
    };

interface PixelRagBridgePreview {
  readonly byte_size: number;
...
export interface PixelRagBridgeMediaAsset {
  readonly asset_id: string;
  readonly content_ref: string;
  readonly content_sha256: string;
  ...
  readonly preview: PixelRagBridgePreview;
}

interface PixelRagBridgeMedia {
  readonly manifest_sha256: string;
  readonly assets: readonly PixelRagBridgeMediaAsset[];
}
...
interface ExpectedPixelRagMediaIdentity {
  readonly asset_id: string;
  readonly content_ref: string;
  readonly content_sha256: string;
  readonly original_height: number;
...
function positiveSafeInteger(value: unknown, path: string): number {
  const resolved = finiteAt(value, path);
...
const MAX_ORIGINAL_MEDIA_BYTES = 24 * 1024 * 1024;
const MAX_ORIGINAL_MEDIA_TOTAL_BYTES = 8 * 1024 * 1024;
const MAX_ORIGINAL_IMAGE_SIDE = 8192;
const MAX_ORIGINAL_IMAGE_PIXELS = 40_000_000;
const MAX_PREVIEW_SIDE = 480;
const MAX_PREVIEW_PIXELS = MAX_PREVIEW_SIDE * MAX_PREVIEW_SIDE;
const MAX_PREVIEW_BYTES = 64 * 1024;
const MAX_PREVIEW_TOTAL_BYTES = 180 * 1024;

function boundedPositiveSafeInteger(value: unknown, path: string, maximum: number): number {
  const resolved = positiveSafeInteger(value, path);
  if (resolved > maximum) throw new Error(`${path} exceeds its maximum ${maximum}`);
  return resolved;
}

function expectedMediaIdentity(value: unknown, path: string): ExpectedPixelRagMediaIdentity {
  const record = objectAt(value, path);
  const image = objectAt(record.image, `${path}.image`);
  return {
    asset_id: stringAt(record.asset_id, `${path}.asset_id`),
    content_ref: khiveAt(record.khive, `${path}.khive`).content_ref,
    content_sha256: shaAt(record.sha256, `${path}.sha256`),
    original_height: positiveSafeInteger(image.height, `${path}.image.height`),
    original_width: positiveSafeInteger(image.width, `${path}.image.width`),
  };
}

function expectedEngineMediaIdentities(artifact: JsonRecord): ReadonlyMap<string, ExpectedPixelRagMediaIdentity> {
  const source = expectedMediaIdentity(artifact.source, "$pixel_rag_bridge.artifact.source");
  const identities = [source];
  for (const [intentIndex, intentValue] of arrayAt(
    artifact.intents,
    "$pixel_rag_bridge.artifact.intents",
  ).entries()) {
    const intentPath = `$pixel_rag_bridge.artifact.intents[${intentIndex}]`;
    const intent = objectAt(intentValue, intentPath);
    const retrieval = objectAt(intent.retrieval, `${intentPath}.retrieval`);
    for (const [hitIndex, hitValue] of arrayAt(
      retrieval.ranked_evidence,
      `${intentPath}.retrieval.ranked_evidence`,
    ).entries()) {
      identities.push(expectedMediaIdentity(
        hitValue,
        `${intentPath}.retrieval.ranked_evidence[${hitIndex}]`,
      ));
    }
  }

  const byId = new Map<string, ExpectedPixelRagMediaIdentity>();
  const idsBySha = new Map<string, string>();
  const idsByContentRef = new Map<string, string>();
  for (const identity of identities) {
    const existing = byId.get(identity.asset_id);
    if (existing) {
      if (
        existing.content_sha256 !== identity.content_sha256
        || existing.content_ref !== identity.content_ref
        || existing.original_width !== identity.original_width
        || existing.original_height !== identity.original_height
      ) {
        throw new Error("$pixel_rag_bridge artifact repeats an asset_id with a different media identity");
      }
      continue;
    }
    const shaOwner = idsBySha.get(identity.content_sha256);
    const refOwner = idsByContentRef.get(identity.content_ref);
    if (shaOwner || refOwner) {
      throw new Error("$pixel_rag_bridge artifact aliases a visible SHA-256 or ContentRef across asset ids");
    }
    byId.set(identity.asset_id, identity);
    idsBySha.set(identity.content_sha256, identity.asset_id);
    idsByContentRef.set(identity.content_ref, identity.asset_id);
  }
  return byId;
}
...
function sha256BinaryString(raw: string): string {
  const paddedLength = Math.ceil((raw.length + 9) / 64) * 64;
...
function canonicalBase64Bytes(value: unknown, path: string, maximumByteSize: number): {
  readonly byteSize: number;
  readonly decoded: string;
  readonly payload: string;
  readonly sha256: string;
} {
  const payload = stringAt(value, path);
  if (payload.length > Math.ceil(maximumByteSize / 3) * 4) {
    throw new Error(`${path} exceeds its decoded byte ceiling`);
  }
  if (
    payload.length % 4 !== 0
    || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/u.test(payload)
  ) {
    throw new Error(`${path} must be canonical padded base64`);
  }
  let decoded: string;
  try {
  ...
  if (btoa(decoded) !== payload) {
    throw new Error(`${path} must be canonical padded base64`);
  }
  return { payload, decoded, byteSize: decoded.length, sha256: sha256BinaryString(decoded) };
}

function expectedPreviewDimensions(originalWidth: number, originalHeight: number): {
  readonly height: number;
  readonly width: number;
} {
  const longestSide = Math.max(originalWidth, originalHeight);
  if (longestSide <= MAX_PREVIEW_SIDE) return { width: originalWidth, height: originalHeight };
  const scale = MAX_PREVIEW_SIDE / longestSide;
  return {
...
function jpegDimensions(bytes: string, path: string): { readonly height: number; readonly width: number } {
  ...
  if (
    ...
    throw new Error(`${path} must contain a complete JPEG payload`);
  }
  ...
  while (offset < bytes.length - 1) {
    if (byte(offset) !== 0xff) throw new Error(`${path} has an invalid JPEG marker boundary`);
    while (offset < bytes.length && byte(offset) === 0xff) offset += 1;
    ...
    const segmentLength = byte(offset) * 256 + byte(offset + 1);
    if (segmentLength < 2 || offset + segmentLength > bytes.length) {
      throw new Error(`${path} has an invalid JPEG segment length`);
    }
    if (startOfFrameMarkers.has(marker)) {
      if (segmentLength < 7) throw new Error(`${path} has an invalid JPEG frame header`);
      const height = byte(offset + 3) * 256 + byte(offset + 4);
      const width = byte(offset + 5) * 256 + byte(offset + 6);
      if (width < 1 || height < 1) throw new Error(`${path} has invalid JPEG dimensions`);
      return { width, height };
    }
    if (marker === 0xda) break;
    offset += segmentLength;
  }
  throw new Error(`${path} does not declare JPEG dimensions`);
}
...
): PixelRagBridgePreview {
  const record = objectAt(value, path);
  closed(
    record,
    ["byte_size", "data_base64", "height", "mime", "sha256", "width"],
    path,
  );
  const mime = exact(record.mime, ["image/jpeg"], `${path}.mime`);
  const byteSize = boundedPositiveSafeInteger(record.byte_size, `${path}.byte_size`, MAX_PREVIEW_BYTES);
  const encoded = canonicalBase64Bytes(record.data_base64, `${path}.data_base64`, MAX_PREVIEW_BYTES);
  if (encoded.byteSize !== byteSize) {
    throw new Error(`${path}.byte_size does not match its decoded base64 payload`);
  }
  const sha256 = shaAt(record.sha256, `${path}.sha256`);
  if (encoded.sha256 !== sha256) {
    throw new Error(`${path}.data_base64 SHA-256 does not match ${path}.sha256`);
  }
  const width = boundedPositiveSafeInteger(record.width, `${path}.width`, MAX_PREVIEW_SIDE);
  const height = boundedPositiveSafeInteger(record.height, `${path}.height`, MAX_PREVIEW_SIDE);
  if (width * height > MAX_PREVIEW_PIXELS) {
    throw new Error(`${path} dimensions exceed the preview pixel ceiling`);
  }
  const expected = expectedPreviewDimensions(originalWidth, originalHeight);
  if (width !== expected.width || height !== expected.height) {
    throw new Error(`${path} dimensions do not match the bounded original geometry`);
  }
  const decoded = jpegDimensions(encoded.decoded, `${path}.data_base64`);
  if (decoded.width !== width || decoded.height !== height) {
    throw new Error(`${path} declared dimensions do not match the JPEG payload`);
  }
  return {
    byte_size: byteSize,
    data_base64: encoded.payload,
    height,
    mime,
    sha256,
    width,
...
function mediaAssetAt(value: unknown, path: string): PixelRagBridgeMediaAsset {
  const record = objectAt(value, path);
  closed(
    record,
    [
  ...
  const originalByteSize = boundedPositiveSafeInteger(
    record.original_byte_size,
    `${path}.original_byte_size`,
    MAX_ORIGINAL_MEDIA_BYTES,
  );
  const originalWidth = boundedPositiveSafeInteger(
    record.original_width,
    `${path}.original_width`,
    MAX_ORIGINAL_IMAGE_SIDE,
  );
  const originalHeight = boundedPositiveSafeInteger(
    record.original_height,
    `${path}.original_height`,
    MAX_ORIGINAL_IMAGE_SIDE,
  );
  if (originalWidth * originalHeight > MAX_ORIGINAL_IMAGE_PIXELS) {
    throw new Error(`${path} original dimensions exceed the pixel ceiling`);
  }
  return {
    asset_id: stringAt(record.asset_id, `${path}.asset_id`),
    content_ref: contentRefAt(record.content_ref, `${path}.content_ref`),
    ...
    preview: previewAt(record.preview, `${path}.preview`, originalWidth, originalHeight),
  };
}

function bridgeMediaAt(value: unknown, artifact: JsonRecord): PixelRagBridgeMedia {
  const path = "$pixel_rag_bridge.media";
  const record = objectAt(value, path);
  closed(record, ["assets", "manifest_sha256"], path);
  const manifestSha256 = shaAt(record.manifest_sha256, `${path}.manifest_sha256`);
  ...
  if (
    ...
    throw new Error(`${path}.manifest_sha256 does not match the artifact source manifest`);
  }

  const expected = expectedEngineMediaIdentities(artifact);
  const assets = arrayAt(record.assets, `${path}.assets`).map((entry, index) =>
    mediaAssetAt(entry, `${path}.assets[${index}]`),
  );
  if (assets.length !== expected.size) {
    throw new Error(`${path}.assets must equal the exact visible artifact media set`);
  }
  const seen = new Set<string>();
  for (const [index, asset] of assets.entries()) {
    if (seen.has(asset.asset_id)) {
      throw new Error(`${path}.assets repeats duplicate asset_id ${asset.asset_id}`);
    }
    seen.add(asset.asset_id);
    if (index > 0 && assets[index - 1]!.asset_id.localeCompare(asset.asset_id) >= 0) {
      throw new Error(`${path}.assets must be uniquely sorted by asset_id`);
    }
    const identity = expected.get(asset.asset_id);
    if (!identity) {
      throw new Error(`${path}.assets contains asset_id outside the exact visible media set`);
    }
    if (asset.content_sha256 !== identity.content_sha256) {
      throw new Error(`${path}.assets ${asset.asset_id} content_sha256 does not match the artifact identity`);
    }
    if (asset.content_ref !== identity.content_ref) {
      throw new Error(`${path}.assets ${asset.asset_id} content_ref does not match the artifact identity`);
    }
    if (
      asset.original_width !== identity.original_width
      || asset.original_height !== identity.original_height
    ) {
      throw new Error(`${path}.assets ${asset.asset_id} original dimensions do not match the artifact identity`);
    }
  }
  for (const assetId of expected.keys()) {
    if (!seen.has(assetId)) {
      throw new Error(`${path}.assets is missing visible artifact identity ${assetId}`);
    }
  }
  const originalByteTotal = assets.reduce((total, asset) => total + asset.original_byte_size, 0);
  if (originalByteTotal > MAX_ORIGINAL_MEDIA_TOTAL_BYTES) {
    throw new Error(`${path}.assets original bytes exceed the aggregate byte ceiling`);
  }
  const previewByteTotal = assets.reduce((total, asset) => total + asset.preview.byte_size, 0);
  if (previewByteTotal > MAX_PREVIEW_TOTAL_BYTES) {
    throw new Error(`${path}.assets preview bytes exceed the aggregate byte ceiling`);
  }
  return { manifest_sha256: manifestSha256, assets };
}
...
function decodePythonPrevalidatedPixelRagBridgeShell(value: unknown): PixelRagBridgeEnvelope {
  ...
  exact(record.format_version, ["moodboard.viewer-pixel-rag-bridge.v2"], "$pixel_rag_bridge.format_version");
  exact(
    record.generator_revision,
    ["moodboard.pixel-rag-viewer-bridge.v2"],
    "$pixel_rag_bridge.generator_revision",
  );
  const state = exact(record.state, ["fallback", "projected"], "$pixel_rag_bridge.state");
  if (state === "fallback") {
    if (record.input !== null || record.artifact !== null || record.media !== null) {
      throw new Error("$pixel_rag_bridge fallback cannot carry input, artifact, or media data");
    }
    return {
      artifact: null,
  ...
  const media = bridgeMediaAt(record.media, artifact);
  return {
    artifact,
    format_version: "moodboard.viewer-pixel-rag-bridge.v2",
    generator_revision: "moodboard.pixel-rag-viewer-bridge.v2",
...
const verifiedPixelRagMediaSources: ReadonlyMap<string, SafeThumbnailSource> = (() => {
  if (pixelRagBridge.state === "fallback") return new Map();
  return new Map(pixelRagBridge.media.assets.map((asset) => [
    asset.content_sha256,
    `data:${asset.preview.mime};base64,${asset.preview.data_base64}` as SafeThumbnailSource,
  ]));
})();

export function verifiedPixelRagMediaIdentity(
  contentSha256: string,
): PixelRagBridgeMediaAsset | undefined {
  if (pixelRagBridge.state === "fallback") return undefined;
  return pixelRagBridge.media.assets.find((asset) => asset.content_sha256 === contentSha256);
}
...
export const __pixelRagTest = {
  decodePythonPrevalidatedPixelRagBridgeShell,
} as const;
