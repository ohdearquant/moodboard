...
describe("Pixel RAG demo artifact", () => {
  ...
  it("resolves every visible measured identity from bridge-owned media with an empty report", () => {
    const identities = [
      verifiedPixelRagArtifact.source,
      ...verifiedPixelRagArtifact.intents.flatMap((intent) => intent.evidence),
    ];
    expect(identities).toHaveLength(7);

    const sources = identities.map((identity) =>
      resolvePixelRagMediaSource(identity, emptyReportModel()),
    );
    expect(sources).toHaveLength(7);
    for (const source of sources) {
      expect(source).toMatch(/^data:image\/(?:jpeg|png|webp);base64,[A-Za-z0-9+/]+=*$/u);
    }
  });

  it("serves only bounded JPEG previews while preserving each original media identity", () => {
    expect(pixelRagBridge.state).toBe("projected");
    if (pixelRagBridge.state !== "projected") return;
    const identities = [
      verifiedPixelRagArtifact.source,
      ...verifiedPixelRagArtifact.intents.flatMap((intent) => intent.evidence),
    ];
    const identityBySha = new Map(identities.map((identity) => [identity.content_sha256, identity]));
    let totalPreviewBytes = 0;

    for (const asset of pixelRagBridge.media.assets as readonly any[]) {
      expect(Object.keys(asset).toSorted()).toEqual([
        "asset_id",
        "content_ref",
        "content_sha256",
        "original_byte_size",
      ...
      expect(Object.keys(asset.preview).toSorted()).toEqual([
        "byte_size",
        "data_base64",
        "height",
        "mime",
      ...
      expect(asset.preview.mime).toBe("image/jpeg");
      expect(asset.preview.byte_size).toBeLessThanOrEqual(64 * 1024);
      expect(Math.max(asset.preview.width, asset.preview.height)).toBeLessThanOrEqual(480);
      totalPreviewBytes += asset.preview.byte_size;

      const identity = identityBySha.get(asset.content_sha256);
      expect(identity, asset.asset_id).toBeDefined();
      expect(asset.content_ref).toBe(identity?.khive.content_ref);
      expect(resolvePixelRagMediaSource(identity!, emptyReportModel())).toBe(
        `data:image/jpeg;base64,${asset.preview.data_base64}`,
      );
    }
    expect(totalPreviewBytes).toBeLessThanOrEqual(180 * 1024);
  });

  it("closes and cross-binds every bridge-owned media identity", () => {
    const decode = __pixelRagTest.decodePythonPrevalidatedPixelRagBridgeShell;
    expect(decode({
      artifact: null,
      format_version: "moodboard.viewer-pixel-rag-bridge.v2",
      generator_revision: "moodboard.pixel-rag-viewer-bridge.v2",
      ...
      media: null,
      state: "fallback",
    })).toMatchObject({ artifact: null, input: null, media: null, state: "fallback" });

    const valid = structuredClone(pixelRagBridge) as any;
    expect(decode(valid)).toMatchObject({ state: "projected", media: { assets: expect.any(Array) } });

    const missingShellField = structuredClone(valid);
    delete missingShellField.media;
    expect(() => decode(missingShellField)).toThrow(/media|closed/i);

    const extraShellField = structuredClone(valid);
    extraShellField.unbound_media = [];
    expect(() => decode(extraShellField)).toThrow(/unknown key/i);

    const duplicate = structuredClone(valid);
    duplicate.media.assets.push(structuredClone(duplicate.media.assets[0]));
    expect(() => decode(duplicate)).toThrow(/duplicate|exact.*media set/i);

    const missingAssetField = structuredClone(valid);
    delete missingAssetField.media.assets[0].preview;
    expect(() => decode(missingAssetField)).toThrow(/preview|missing|object/i);

    const extraAssetField = structuredClone(valid);
    extraAssetField.media.assets[0].raw_bytes = "forbidden";
    expect(() => decode(extraAssetField)).toThrow(/unknown key/i);

    const extraPreviewField = structuredClone(valid);
    extraPreviewField.media.assets[0].preview.unbound_metadata = true;
    expect(() => decode(extraPreviewField)).toThrow(/unknown key/i);

    for (const [label, mutate, message] of [
      ["asset id", (bridge: any) => { bridge.media.assets[0].asset_id = "unbound-asset"; }, /asset_id|identity|media set/i],
      ["original SHA-256", (bridge: any) => { bridge.media.assets[0].content_sha256 = "f".repeat(64); }, /content_sha256|identity/i],
      ["ContentRef", (bridge: any) => { bridge.media.assets[0].content_ref = "e".repeat(64); }, /content_ref|identity/i],
      ["original width", (bridge: any) => { bridge.media.assets[0].original_width += 1; }, /original_width|dimension|identity/i],
      ["original byte ceiling", (bridge: any) => { bridge.media.assets[0].original_byte_size = 24 * 1024 * 1024 + 1; }, /original_byte_size|byte/i],
      ["preview width", (bridge: any) => { bridge.media.assets[0].preview.width += 1; }, /preview.*width|dimension/i],
      ["preview MIME", (bridge: any) => { bridge.media.assets[0].preview.mime = "image/png"; }, /preview.*mime/i],
      ["preview byte ceiling", (bridge: any) => { bridge.media.assets[0].preview.byte_size = 64 * 1024 + 1; }, /preview.*byte_size|byte/i],
      ["preview SHA-256", (bridge: any) => { bridge.media.assets[0].preview.sha256 = "f".repeat(64); }, /preview.*sha256|SHA-256/i],
      ["preview base64", (bridge: any) => {
        const payload = bridge.media.assets[0].preview.data_base64 as string;
        bridge.media.assets[0].preview.data_base64 = `${payload[0] === "A" ? "B" : "A"}${payload.slice(1)}`;
      }, /base64|SHA-256/i],
      ["manifest", (bridge: any) => { bridge.media.manifest_sha256 = "d".repeat(64); }, /manifest_sha256/i],
    ] as const) {
      const drifted = structuredClone(valid);
      mutate(drifted);
      expect(() => decode(drifted), label).toThrow(message);
    }
  });
});
